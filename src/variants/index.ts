...
import {
    ...
    motionReduceVariant,
    motionSafeVariant,
    orientationLandscapeVariant,
    orientationPortraitVariant,
    printVariant,
} from './media';
import {
    makeAriaVariants,
...
export default function makeVariants(): Variant<PresetConfig> {
    return (matcher, c) => {
        ...
        const resolvePluginVariantReturn = (r: PluginVariant) => {
            ...
            if (r?.selectorComplements)
                config.selectorComplements.push(...r.selectorComplements);
        ...
        return {
            ...
            parent: `@media ${config.mediaQuery
                .map((media) => `(${media})`)
                .join(' and ')}`,
        };
    };
